...
export default function RealEstatePage ({title, current_savings, target_amount,color}) {
    ...
    return(
        <div className="w-full bg-white shadow-[1px_1px_5px_rgba(0,0,0,0.05)] rounded-[6px] p-4 flex flex-col gap-3">
            <div className="w-full flex flex-row">
                ...
                <img src={Realestate} alt="부동산 이미지" />
            </div>
            ...
            <div className="flex flex-row justify-between w-full">
                <div>
                    <span style={{color:color}} className="text-left font-semibold text-xl">{current_savings}</span> / <span style={{color:"#5A9821"}}  className="text-left font-semibold text-xl">{target_amount}</span>
                </div >
          ...
          <div className="w-full h-[24px] bg-[#F1EFEF] border border-[#DFDFDF] rounded-[70px]">
            ...
            <div
              ...
              style={{ width: `${currentProgress}%`, backgroundColor:"#5A9821" }}
            ></div>
          </div>
        </div>
    )
