import DigitalThing from "../../assets/DigitalThing.png"
import MoreIcon from "../../assets/MoreIcon.png"

export default function DigitalDevicePage ({title, current_savings, target_amount,color, onSaving, bucketListId, onDelete}) {
    ...
    return(
        <div className="w-full bg-white rounded-2xl shadow-md p-4 flex flex-col gap-3 relative">
            ...
            <div className="w-full flex flex-row">
                ...
                <img src={DigitalThing} alt="디지털 기기 이미지" className="w-full" />
            </div>
            ...
            <div className="flex flex-row justify-between w-full">
                <div>
                    <span style={{color:color}} className="text-left font-semibold text-xl">{current_savings.toLocaleString()}</span> / <span style={{color:"#321B41"}}  className="text-left font-semibold text-xl">{target_amount.toLocaleString()}</span>
                </div>
          ...
          <div className="w-full h-[24px] bg-[#F1EFEF] border border-[#DFDFDF] rounded-[70px]">
            ...
            <div
              ...
              style={{ width: `${currentProgress}%`, backgroundColor:"#321B41" }}
            ></div>
          </div>
        </div>
    )
