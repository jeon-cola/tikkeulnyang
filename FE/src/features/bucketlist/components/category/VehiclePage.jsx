...
export default function VehiclePage({title, current_savings, target_amount ,color, onSaving, bucketListId, onDelete}) {
    ...
    return(
         <div className="w-full bg-white rounded-2xl shadow-md p-4 flex flex-col gap-3 relative">
            ...
            <div className="flex flex-row justify-between w-full">
                <div>
                    <span style={{color:color}} className="text-left font-semibold text-xl">{current_savings.toLocaleString()}</span> / <span style={{color:"#FF7452"}}  className="text-left font-semibold text-xl">{target_amount.toLocaleString()}</span>
                </div >
          ...
          <div className="w-full h-[24px] bg-[#F1EFEF] border border-[#DFDFDF] rounded-[70px]">
            ...
            <div
              ...
              style={{ width: `${currentProgress}%`, backgroundColor:"#FF7452" }}
            ></div>
          </div>
        </div>
    )
